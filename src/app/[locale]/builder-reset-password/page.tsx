...
import { BuilderResetPasswordForm } from "@/features/auth/components/builder-reset-password-form";
import { Link } from "@/i18n/navigation";

type P = { locale: string } | Promise<{ locale: string }>;

export default async function BuilderResetPasswordPage({ params }: Readonly<{ params: P }>) {
  const { locale } = await Promise.resolve(params);
  setRequestLocale(locale);
  const t = await getTranslations({ locale, namespace: "BuilderReset" });
  return (
    <div className="relative flex min-h-[78vh] flex-col items-center justify-center overflow-hidden px-4 py-16">
      <div
        ...
        aria-hidden
      />
      <Suspense fallback={<div className="h-64 w-full max-w-md animate-pulse rounded-2xl bg-muted/50" />}>
        <BuilderResetPasswordForm />
      </Suspense>
